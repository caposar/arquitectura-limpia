...
namespace DientesLimpios.Infraestructura.Notificaciones
{
    public class ServicioCorreos : IServicioNotificaciones
    {
        ...
        Su cita con el Dr (Dra.) {cita.Dentista} ha sido programada para el {cita.Fecha.ToString("f", new CultureInfo("es-AR"))} en el consultorio {cita.Consultorio}.
        ...
        public async Task EnviarRecordatorioCita(RecordatorioCitaDTO cita)
        {
            ...
            Le recordamos que tiene cita con el Dr (Dra.) {cita.Dentista} para el {cita.Fecha.ToString("f", new CultureInfo("es-AR"))} en el consultorio {cita.Consultorio}.
